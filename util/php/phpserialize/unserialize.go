...
func (self *UnSerializer) readLen() int {
	...
	if raw, err = self.readUntil(SEPARATOR_VALUE_TYPE); err != nil {
		...
		if val, err = strconv.Atoi(raw); err != nil {
			...
			self.saveError(fmt.Errorf("phpserialize: Unserializable object length looks too big(%d). If you are sure you wanna unserialise it, please increase UNSERIAZABLE_OBJECT_MAX_LEN const", val, err))
			val = 0
		}
	}
	return val
