...
// WriteConstants add a const () declaration to the current position in the
// buffer. A line includes the const name equal value.
func (g *Go) WriteConstants(lines ...string) {
	fmt.Fprintln(g.Buffer, "const (")
	sort.Strings(lines)
	for _, cs := range lines {
		fmt.Fprintln(g.Buffer, cs)
	}
	fmt.Fprintln(g.Buffer, ")")
}
...
func (g *Go) GenerateFile(w io.Writer) error {
	...
	fmt.Fprintln(&buf, `// Code generated by corestoreio/pkg/util/codegen. DO NOT EDIT.`)
	if len(g.SecondLineComments) > 0 {
		comment(&buf, g.SecondLineComments...)
	}
