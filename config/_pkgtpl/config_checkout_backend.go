...
func (pp *PkgBackend) init(cfgStruct element.SectionSlice) *PkgBackend {
	pp.Lock()
	defer pp.Unlock()
	pp.CheckoutOptionsOnepageCheckoutEnabled = model.NewBool(`checkout/options/onepage_checkout_enabled`, model.WithFieldFromSectionSlice(cfgStruct))
	pp.CheckoutOptionsGuestCheckout = model.NewBool(`checkout/options/guest_checkout`, model.WithFieldFromSectionSlice(cfgStruct))
	pp.CheckoutCartDeleteQuoteAfter = model.NewStr(`checkout/cart/delete_quote_after`, model.WithFieldFromSectionSlice(cfgStruct))
	pp.CheckoutCartRedirectToCart = model.NewBool(`checkout/cart/redirect_to_cart`, model.WithFieldFromSectionSlice(cfgStruct))
	pp.CheckoutCartLinkUseQty = model.NewStr(`checkout/cart_link/use_qty`, model.WithFieldFromSectionSlice(cfgStruct))
	pp.CheckoutSidebarDisplay = model.NewBool(`checkout/sidebar/display`, model.WithFieldFromSectionSlice(cfgStruct))
	pp.CheckoutSidebarCount = model.NewStr(`checkout/sidebar/count`, model.WithFieldFromSectionSlice(cfgStruct))
	pp.CheckoutPaymentFailedIdentity = model.NewStr(`checkout/payment_failed/identity`, model.WithFieldFromSectionSlice(cfgStruct))
	pp.CheckoutPaymentFailedReceiver = model.NewStr(`checkout/payment_failed/receiver`, model.WithFieldFromSectionSlice(cfgStruct))
	pp.CheckoutPaymentFailedTemplate = model.NewStr(`checkout/payment_failed/template`, model.WithFieldFromSectionSlice(cfgStruct))
	pp.CheckoutPaymentFailedCopyTo = model.NewStr(`checkout/payment_failed/copy_to`, model.WithFieldFromSectionSlice(cfgStruct))
	pp.CheckoutPaymentFailedCopyMethod = model.NewStr(`checkout/payment_failed/copy_method`, model.WithFieldFromSectionSlice(cfgStruct))
