...
func (m *scopedManager) GetString(paths ...string) string {
	...
	if m.storeID > 0 {
		if v, err := m.root.GetString(ScopeStore(m.storeID), argP); err == nil {
			...
			PkgLog.Debug("config.scopedManager.GetString.store.error", "err", err, "scopedManager", m, "paths", paths)
		}
	}
	if m.groupID > 0 {
		if v, err := m.root.GetString(ScopeGroup(m.groupID), argP); err == nil {
			...
			PkgLog.Debug("config.scopedManager.GetString.group.error", "err", err, "scopedManager", m, "paths", paths)
		}
	}
	if m.websiteID > 0 {
		if v, err := m.root.GetString(ScopeWebsite(m.websiteID), argP); err == nil {
			...
			PkgLog.Debug("config.scopedManager.GetString.website.error", "err", err, "scopedManager", m, "paths", paths)
		}
	}
	v, err := m.root.GetString(argP)
	if err != nil && err != ErrKeyNotFound && PkgLog.IsDebug() {
		PkgLog.Debug("config.scopedManager.GetString.default.error", "err", err, "scopedManager", m, "paths", paths)
	}
	return v
}
...
func (m *scopedManager) GetBool(paths ...string) bool {
	...
	if m.storeID > 0 {
		if v, err := m.root.GetBool(ScopeStore(m.storeID), argP); err == nil {
			...
			PkgLog.Debug("config.scopedManager.GetBool.store.error", "err", err, "scopedManager", m, "paths", paths)
		}
	}
	if m.groupID > 0 {
		if v, err := m.root.GetBool(ScopeGroup(m.groupID), argP); err == nil {
			...
			PkgLog.Debug("config.scopedManager.GetBool.group.error", "err", err, "scopedManager", m, "paths", paths)
		}
	}
	if m.websiteID > 0 {
		if v, err := m.root.GetBool(ScopeWebsite(m.websiteID), argP); err == nil {
			...
			PkgLog.Debug("config.scopedManager.GetBool.website.error", "err", err, "scopedManager", m, "paths", paths)
		}
	}
	v, err := m.root.GetBool(argP)
	if err != nil && err != ErrKeyNotFound && PkgLog.IsDebug() {
		PkgLog.Debug("config.scopedManager.GetBool.default.error", "err", err, "scopedManager", m, "paths", paths)
	}
	return v
}
...
func (m *scopedManager) GetFloat64(paths ...string) float64 {
	...
	if m.storeID > 0 {
		if v, err := m.root.GetFloat64(ScopeStore(m.storeID), argP); err == nil {
			...
			PkgLog.Debug("config.scopedManager.GetFloat64.store.error", "err", err, "scopedManager", m, "paths", paths)
		}
	}
	if m.groupID > 0 {
		if v, err := m.root.GetFloat64(ScopeGroup(m.groupID), argP); err == nil {
			...
			PkgLog.Debug("config.scopedManager.GetFloat64.group.error", "err", err, "scopedManager", m, "paths", paths)
		}
	}
	if m.websiteID > 0 {
		if v, err := m.root.GetFloat64(ScopeWebsite(m.websiteID), argP); err == nil {
			...
			PkgLog.Debug("config.scopedManager.GetFloat64.website.error", "err", err, "scopedManager", m, "paths", paths)
		}
	}
	v, err := m.root.GetFloat64(argP)
	if err != nil && err != ErrKeyNotFound && PkgLog.IsDebug() {
		PkgLog.Debug("config.scopedManager.GetFloat64.default.error", "err", err, "scopedManager", m, "paths", paths)
	}
	return v
}
...
func (m *scopedManager) GetInt(paths ...string) int {
	...
	if m.storeID > 0 {
		if v, err := m.root.GetInt(ScopeStore(m.storeID), argP); err == nil {
			...
			PkgLog.Debug("config.scopedManager.GetInt.store.error", "err", err, "scopedManager", m, "paths", paths)
		}
	}
	if m.groupID > 0 {
		if v, err := m.root.GetInt(ScopeGroup(m.groupID), argP); err == nil {
			...
			PkgLog.Debug("config.scopedManager.GetInt.group.error", "err", err, "scopedManager", m, "paths", paths)
		}
	}
	if m.websiteID > 0 {
		if v, err := m.root.GetInt(ScopeWebsite(m.websiteID), argP); err == nil {
			...
			PkgLog.Debug("config.scopedManager.GetInt.website.error", "err", err, "scopedManager", m, "paths", paths)
		}
	}
	v, err := m.root.GetInt(argP)
	if err != nil && err != ErrKeyNotFound && PkgLog.IsDebug() {
		PkgLog.Debug("config.scopedManager.GetInt.default.error", "err", err, "scopedManager", m, "paths", paths)
	}
	return v
}
...
func (m *scopedManager) GetDateTime(paths ...string) time.Time {
	...
	if m.storeID > 0 {
		if v, err := m.root.GetDateTime(ScopeStore(m.storeID), argP); err == nil {
			...
			PkgLog.Debug("config.scopedManager.GetDateTime.store.error", "err", err, "scopedManager", m, "paths", paths)
		}
	}
	if m.groupID > 0 {
		if v, err := m.root.GetDateTime(ScopeGroup(m.groupID), argP); err == nil {
			...
			PkgLog.Debug("config.scopedManager.GetDateTime.group.error", "err", err, "scopedManager", m, "paths", paths)
		}
	}
	if m.websiteID > 0 {
		if v, err := m.root.GetDateTime(ScopeWebsite(m.websiteID), argP); err == nil {
			...
			PkgLog.Debug("config.scopedManager.GetDateTime.website.error", "err", err, "scopedManager", m, "paths", paths)
		}
	}
	v, err := m.root.GetDateTime(argP)
	if err != nil && err != ErrKeyNotFound && PkgLog.IsDebug() {
		PkgLog.Debug("config.scopedManager.GetDateTime.default.error", "err", err, "scopedManager", m, "paths", paths)
	}
	return v
}
