...
// HTTPHandlerOptions can set different behaviour to the Register/Deregister
// endpoints.
...
// HTTPJSONObserverRegistry provides an endpoint handler to register validators
// with the concrete type of config.Service
func HTTPJSONObserverRegistry(or config.ObserverRegisterer, ho HTTPHandlerOptions) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := RegisterObservers(or, io.LimitReader(r.Body, ho.MaxRequestSize)); err != nil {
...
// HTTPJSONObserverDeregistry provides an endpoint handler to deregister
// validators with the concrete type of config.Service
func HTTPJSONObserverDeregistry(or config.ObserverRegisterer, ho HTTPHandlerOptions) http.Handler {
	if ho.StatusCodeOk == 0 {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err := DeregisterObservers(or, io.LimitReader(r.Body, ho.MaxRequestSize)); err != nil {
			ho.ErrorHandler(err).ServeHTTP(w, r)
			return
		}
		// io.Copy(ioutil.Discard, r.Body) // I don't know if that is necessary
