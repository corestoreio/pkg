...
func TestPubSubBubbling(t *testing.T) {
	...
	assert.NoError(t, s.Write(testPath.BindWebsite(123), []byte(`1`)))
	...
	assert.NoError(t, s.Write(testPath2.BindWebsite(3), []byte(`1`)))
	err = s.Close()
...
func TestPubSubPanicSimple(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, 1, subID, "The very first subscription ID should be 1")
	assert.NoError(t, s.Write(testPath.BindStore(123), []byte(`321`)), "Writing value 123 should not fail")
...
func TestPubSubPanicError(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, 1, subID, "The very first subscription ID should be 1")
	assert.NoError(t, s.Write(testPath.BindStore(123), []byte(`321`)))
...
func TestPubSubUnsubscribe(t *testing.T) {
	...
	assert.NoError(t, s.Write(p, []byte(`any kind of data`)))
	assert.NoError(t, s.Close())
