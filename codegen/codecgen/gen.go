...
func Generate(outfile, buildTag, codecPkgPath string, useUnsafe bool, goRunTag string,
	...
	frunBaseDir, _ := filepath.Split(outfile) // (CS) added base dir or go run would fail

	frunMainName := frunBaseDir + "codecgen-main-" + tv.RandString + ".generated.go"
	frunPkgName := frunBaseDir + "codecgen-pkg-" + tv.RandString + ".generated.go"
	if deleteTempFile {
