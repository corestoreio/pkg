...
func TestStoreCodeFromClaimFullToken(t *testing.T) {
	...
	storenet.CodeAddToClaim(s, token.Claims)

	so, err := storenet.CodeFromClaim(token.Claims)
	assert.NoError(t, err)
	assert.EqualValues(t, "de", so.StoreCode())

	so, err = storenet.CodeFromClaim(nil)
	...
	so, err = storenet.CodeFromClaim(token2.Claims)
...
func TestStoreCodeFromClaimNoToken(t *testing.T) {
	...
	for i, test := range tests {
		so, err := storenet.CodeFromClaim(test.token)
