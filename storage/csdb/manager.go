...
type (
	...
	TableService struct {
		// MultiErr contains errors generated by With* functional options.
		// Can be nil.
		*errors.MultiErr
		initDone bool
		mu       sync.RWMutex
...
func WithTable(idx Index, name string, cols ...Column) ManagerOption {
	return func(tm *TableService) {
		...
		if err := IsValidIdentifier(name); err != nil {
			tm.MultiErr = tm.AppendErrors(err)
		}
...
func (tm *TableService) Structure(i Index) (*Table, error) {
	...
	return nil, errors.NewNotFoundf("[csdb] Table at index %d", i)
}
...
func (tm *TableService) Append(i Index, ts *Table) error {
	if ts == nil {
		return errors.NewFatalf("[csdb] Table pointer cannot be nil for Index %d", i)
	}
	tm.mu.Lock()
	tm.ts[i] = ts
...
func (tm *TableService) Init(dbrSess dbr.SessionRunner, reInit ...bool) error {
	...
	if false == tm.initDone && true == reLoad {
		return errors.NewTemporaryf("[csdb] You cannot force reload when the init process were not able to run.")
	}
	tm.initDone = true
