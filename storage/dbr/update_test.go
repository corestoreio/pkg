...
func TestUpdateKeywordColumnName(t *testing.T) {
	...
	_, err = s.SelectFrom("dbr_people").Star().Where(Column("email").Str("ben@whitehouse.gov")).Load(context.TODO(), &person)
	assert.NoError(t, err)
...
func TestUpdateReal(t *testing.T) {
	...
	var person dbrPerson
	_, err = s.SelectFrom("dbr_people").Star().Where(Column("id").Int64(id)).Load(context.TODO(), &person)
	assert.NoError(t, err)
...
func TestUpdate_SetRecord(t *testing.T) {
	...
	t.Run("without where", func(t *testing.T) {
		u := NewUpdate("dbr_person").AddColumns("name", "email").BindRecord(Qualify("", pRec))
		compareToSQL(t, u, nil,
