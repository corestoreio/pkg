...
func (ce catalogCategoryEntity) appendBind(args dbr.Arguments, column string) (_ dbr.Arguments, err error) {
	...
	default:
		return nil, errors.NewNotFoundf("[dbr_test] %T: Column %q not found", ce, column)
	}
	return args, nil
}
...
func (ts tableStore) appendBind(args dbr.Arguments, column string) (_ dbr.Arguments, err error) {
	...
	default:
		return nil, errors.NewNotFoundf("[dbr_test] %T: Column %q not found", ts, column)
	}
	return args, nil
}
...
func ExampleSelect_BindRecord() {
	...
	s := dbr.NewSelect("t_d.attribute_id", "e.entity_id").
		...
		Where(
			dbr.Column("e.entity_id").In().PlaceHolder(),                      // 678
			dbr.Column("t_d.attribute_id").In().Int64s(45),                    // 45
