...
func init() {
	flag.BoolVar(&runIntegration, "integration", false, "Enables dbr integration tests")
}
...
func BenchmarkInsert_Prepared(b *testing.B) {
	...
	truncate := func(db dbr.Execer) {
		if _, err := db.ExecContext(context.TODO(), "TRUNCATE TABLE `dbr_people`"); err != nil {
			b.Fatal(err)
		}
	}
	...
	stmt, err := c.InsertInto("dbr_people").
