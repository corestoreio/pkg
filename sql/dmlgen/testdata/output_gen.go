...
func NewTables(ctx context.Context, db dml.QueryExecPreparer, opts ...ddl.TableOption) (tm *ddl.Tables, err error) {
	if tm, err = ddl.NewTables(
		ddl.WithCreateTable(ctx, db, TableNameCoreConfigData, ""),
		ddl.WithCreateTable(ctx, db, TableNameCustomerEntity, ""),
		ddl.WithCreateTable(ctx, db, TableNameDmlgenTypes, ""),
	); err != nil {
...
type CoreConfigData struct {
	...
	Expires   time.Time   `json:"expires,omitempty"`       // expires timestamp NOT NULL  DEFAULT ''0000-00-00 00:00:00''  "Value expiration time"
