...
func TestManualNewDBManager_Tuples_SalesOrderStatusState(t *testing.T) {
	...
	t.Run("Events and cached queries", func(t *testing.T) {
		queries := conv.ToStringSlice(dbm.ConnPool.CachedQueries())

		wantSQLQueries := []string{
			"CatalogProductIndexEAVDecimalIDXDeleteByPK",
			"DELETE FROM `catalog_product_index_eav_decimal_idx` WHERE ((`entity_id`, `attribute_id`, `store_id`, `source_id`) IN /*TUPLES=004*/)",
			"CatalogProductIndexEAVDecimalIDXInsert",
			"INSERT INTO `catalog_product_index_eav_decimal_idx` (`entity_id`,`attribute_id`,`store_id`,`source_id`,`value`) VALUES ",
			"CatalogProductIndexEAVDecimalIDXSelectByPK",
			"SELECT `entity_id`, `attribute_id`, `store_id`, `source_id`, `value` FROM `catalog_product_index_eav_decimal_idx` AS `main_table` WHERE ((`entity_id`, `attribute_id`, `store_id`, `source_id`) = /*TUPLES=004*/) LIMIT 0,1000",
			"CatalogProductIndexEAVDecimalIDXUpdateByPK",
			"UPDATE `catalog_product_index_eav_decimal_idx` SET `entity_id`=?, `attribute_id`=?, `store_id`=?, `source_id`=?, `value`=? WHERE ((`entity_id`, `attribute_id`, `store_id`, `source_id`) = /*TUPLES=004*/)",
			"CatalogProductIndexEAVDecimalIDXUpsertByPK",
			"INSERT INTO `catalog_product_index_eav_decimal_idx` (`entity_id`,`attribute_id`,`store_id`,`source_id`,`value`) VALUES  ON DUPLICATE KEY UPDATE `entity_id`=VALUES(`entity_id`), `attribute_id`=VALUES(`attribute_id`), `store_id`=VALUES(`store_id`), `source_id`=VALUES(`source_id`), `value`=VALUES(`value`)",
			"CatalogProductIndexEAVDecimalIDXesSelectAll",
			"SELECT `entity_id`, `attribute_id`, `store_id`, `source_id`, `value` FROM `catalog_product_index_eav_decimal_idx` AS `main_table` LIMIT 0,1000",
			"CatalogProductIndexEAVDecimalIDXesSelectByPK",
			"SELECT `entity_id`, `attribute_id`, `store_id`, `source_id`, `value` FROM `catalog_product_index_eav_decimal_idx` AS `main_table` WHERE ((`entity_id`, `attribute_id`, `store_id`, `source_id`) IN /*TUPLES=004*/) LIMIT 0,1000",
			"CoreConfigurationDeleteByPK",
			"DELETE FROM `core_configuration` WHERE (`config_id` IN ?)",
			"CoreConfigurationInsert",
			"INSERT INTO `core_configuration` (`scope`,`scope_id`,`expires`,`path`,`value`) VALUES ",
			"CoreConfigurationSelectByPK",
			"SELECT `config_id`, `scope`, `scope_id`, `expires`, `path`, `value` FROM `core_configuration` AS `main_table` WHERE (`config_id` = ?) LIMIT 0,1000",
			"CoreConfigurationUpdateByPK",
			"UPDATE `core_configuration` SET `scope`=?, `scope_id`=?, `expires`=?, `path`=?, `value`=? WHERE (`config_id` = ?)",
			"CoreConfigurationUpsertByPK",
			"INSERT INTO `core_configuration` (`scope`,`scope_id`,`expires`,`path`,`value`) VALUES  ON DUPLICATE KEY UPDATE `scope`=VALUES(`scope`), `scope_id`=VALUES(`scope_id`), `expires`=VALUES(`expires`), `path`=VALUES(`path`), `value`=VALUES(`value`)",
			"CoreConfigurationsSelectAll",
			"SELECT `config_id`, `scope`, `scope_id`, `expires`, `path`, `value` FROM `core_configuration` AS `main_table` LIMIT 0,1000",
			"CoreConfigurationsSelectByPK",
			"SELECT `config_id`, `scope`, `scope_id`, `expires`, `path`, `value` FROM `core_configuration` AS `main_table` WHERE (`config_id` IN ?) LIMIT 0,1000",
			"CustomerAddressEntitiesSelectAll",
			...
			"CustomerAddressEntitiesSelectByPK",
			...
			"CustomerAddressEntityDeleteByPK",
			"DELETE FROM `customer_address_entity` WHERE (`entity_id` IN ?)",
			"CustomerAddressEntityInsert",
			"INSERT INTO `customer_address_entity` (`increment_id`,`parent_id`,`is_active`,`city`,`company`,`country_id`,`fax`,`firstname`,`lastname`,`middlename`,`postcode`,`prefix`,`region`,`region_id`,`street`,`suffix`,`telephone`,`vat_id`,`vat_is_valid`,`vat_request_date`,`vat_request_id`,`vat_request_success`) VALUES ",
			...
			"SalesOrderStatusStateDeleteByPK",
			"DELETE FROM `sales_order_status_state` WHERE ((`status`, `state`) IN /*TUPLES=002*/)",
			"SalesOrderStatusStateInsert",
			"INSERT INTO `sales_order_status_state` (`status`,`state`,`is_default`,`visible_on_front`) VALUES ",
			"SalesOrderStatusStateSelectByPK",
			"SELECT `status`, `state`, `is_default`, `visible_on_front` FROM `sales_order_status_state` AS `main_table` WHERE ((`status`, `state`) = /*TUPLES=002*/) LIMIT 0,1000",
			"SalesOrderStatusStateUpdateByPK",
			"UPDATE `sales_order_status_state` SET `status`=?, `state`=?, `is_default`=?, `visible_on_front`=? WHERE ((`status`, `state`) = /*TUPLES=002*/)",
			"SalesOrderStatusStateUpsertByPK",
			"INSERT INTO `sales_order_status_state` (`status`,`state`,`is_default`,`visible_on_front`) VALUES  ON DUPLICATE KEY UPDATE `status`=VALUES(`status`), `state`=VALUES(`state`), `is_default`=VALUES(`is_default`), `visible_on_front`=VALUES(`visible_on_front`)",
			"SalesOrderStatusStatesSelectAll",
			"SELECT `status`, `state`, `is_default`, `visible_on_front` FROM `sales_order_status_state` AS `main_table` LIMIT 0,1000",
			"SalesOrderStatusStatesSelectByPK",
			"SELECT `status`, `state`, `is_default`, `visible_on_front` FROM `sales_order_status_state` AS `main_table` WHERE ((`status`, `state`) IN /*TUPLES=002*/) LIMIT 0,1000",
			"ViewCustomerAutoIncrementSelectByPK",
			"SELECT `ce_entity_id`, `email`, `firstname`, `lastname`, `city` FROM `view_customer_auto_increment` AS `main_table` WHERE (`ce_entity_id` = ?) LIMIT 0,1000",
			"ViewCustomerAutoIncrementsSelectAll",
			"SELECT `ce_entity_id`, `email`, `firstname`, `lastname`, `city` FROM `view_customer_auto_increment` AS `main_table` LIMIT 0,1000",
			"ViewCustomerAutoIncrementsSelectByPK",
			"SELECT `ce_entity_id`, `email`, `firstname`, `lastname`, `city` FROM `view_customer_auto_increment` AS `main_table` WHERE (`ce_entity_id` IN ?) LIMIT 0,1000",
		}
