...
var _ dml.Scanner = (*salesCreditMemoCollection)(nil)
...
func (cc *salesCreditMemoCollection) RowScan(r *sql.Rows) error {
	...
	for i, col := range cc.Convert.Columns {
		...
		if err != nil {
			return errors.Wrapf(err, "[dml] Failed to convert value at row % with column index %d", cc.Convert.Count, i)
		}
	}
...
// type salesCreditMemoCollection which implements interface dml.Scanner. The
