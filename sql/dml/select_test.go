...
func TestSelectJoin(t *testing.T) {
	...
	t.Run("inner, distinct, no cache, high prio", func(t *testing.T) {
		...
		compareToSQL(t, sqlObj, nil,
			"SELECT DISTINCT STRAIGHT_JOIN SQL_NO_CACHE `p1`.*, `p2`.* FROM `dml_people` AS `p1` INNER JOIN `dml_people` AS `p2` ON (`p2`.`id` = `p1`.`id`) AND (`p1`.`id` = ?)",
	...
	t.Run("inner", func(t *testing.T) {
		...
		compareToSQL(t, sqlObj, nil,
			"SELECT `p1`.*, `p2`.* FROM `dml_people` AS `p1` INNER JOIN `dml_people` AS `p2` ON (`p2`.`id` = `p1`.`id`) AND (`p1`.`id` = ?)",
	...
	t.Run("left", func(t *testing.T) {
		...
		compareToSQL(t, sqlObj, nil,
			"SELECT `p1`.*, `p2`.`name` FROM `dml_people` AS `p1` LEFT JOIN `dml_people` AS `p2` ON (`p2`.`id` = `p1`.`id`) AND (`p1`.`id` = ?)",
...
func TestSelect_SetRecord(t *testing.T) {
	...
	t.Run("multiple args from record", func(t *testing.T) {
		...
		compareToSQL(t, sel, nil,
			"SELECT `a`, `b` FROM `dml_person` AS `dp` INNER JOIN `dml_group` AS `dg` ON (`dp`.`id` = ?) WHERE ((`name` = ?) OR (`e` = ?)) AND (`f` <= ?) AND (`g` > ?) AND (`h` IN (?,?,?)) GROUP BY `ab` HAVING (`email` = ?) AND (`n` = ?) ORDER BY `l`",
			"",
	...
	t.Run("single arg JOIN", func(t *testing.T) {
		...
		compareToSQL(t, sel, nil,
			"SELECT `a` FROM `dml_people` AS `dp` INNER JOIN `dml_group` AS `dg` ON (`dp`.`id` = ?) AND (`dg`.`name` = ?) ORDER BY `id`",
			"SELECT `a` FROM `dml_people` AS `dp` INNER JOIN `dml_group` AS `dg` ON (`dp`.`id` = 6666) AND (`dg`.`name` = 'XY%') ORDER BY `id`",
			int64(6666), "XY%",
		)
	})
	t.Run("single arg WHERE", func(t *testing.T) {
		...
		compareToSQL(t, sel, nil,
			...
			"SELECT `a` FROM `dml_people` WHERE (`id` = 6666) ORDER BY `id`",
			int64(6666),
		)
	})
	t.Run("HAVING", func(t *testing.T) {
		...
		compareToSQL(t, sel, nil,
			...
			"SELECT `a` FROM `dml_people` HAVING (`id` = 6666) AND (`name` LIKE 'Hans Wurst') ORDER BY `id`",
