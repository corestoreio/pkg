...
// flushCache signed cache flusher
...
func (s *Service) configFromContext(w http.ResponseWriter, r *http.Request) (scpCfg ScopedConfig) {
	...
	if err != nil {
		s.ErrorHandler(errors.Wrap(err, "[signed] FromContextRequestedStore")).ServeHTTP(w, r)
		return
	}
	...
	if err := scpCfg.IsValid(); err != nil {
		...
		// made a mistake.
		if s.Log.IsDebug() {
			s.Log.Debug("signed.Service.WithRateLimit.configByScopedGetter.Error",
				log.Err(err),
		...
		s.ErrorHandler(errors.Wrap(err, "[signed] ConfigByScopedGetter")).ServeHTTP(w, r)
		return
	}
	return
}
...
func (s *Service) configByScopedGetter(scpGet config.ScopedGetter) ScopedConfig {
	...
	if s.optionFactory != nil {
		res, ok := <-s.optionInflight.DoChan(current.String(), func() (interface{}, error) {
			if err := s.Options(s.optionFactory(scpGet)...); err != nil {
				return newScopedConfigError(errors.Wrap(err, "[signed] Options applied by OptionFactoryFunc")), nil
			}
		...
		if !ok { // unlikely to happen but you'll never know. how to test that?
			return newScopedConfigError(errors.NewFatalf("[signed] Inflight.DoChan returned a closed/unreadable channel"))
		}
		if res.Err != nil {
			return newScopedConfigError(errors.Wrap(res.Err, "[signed] Inflight.DoChan.Error"))
		}
		sCfg, ok := res.Val.(ScopedConfig)
		if !ok {
			sCfg = newScopedConfigError(errors.NewFatalf("[signed] Inflight.DoChan res.Val cannot be type asserted to scopedConfig"))
		}
		return sCfg
	}
	...
	if s.Log.IsDebug() {
		s.Log.Debug("signed.Service.ConfigByScopedGetter.Fallback",
			log.Stringer("requested_scope", current),
			log.Stringer("requested_fallback_scope", fallback),
			log.Stringer("responded_scope", sCfg.ScopeHash),
			log.ErrWithKey("responded_scope_valid", sCfg.IsValid()),
