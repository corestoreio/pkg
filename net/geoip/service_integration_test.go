...
func TestService_WithCountryByIP_Error_GeoReader(t *testing.T) {
	...
	if err := s.Options(
		...
		geoip.WithErrorHandler(scope.Default, 0, func(_ error) http.Handler {
...
func TestService_WithIsCountryAllowedByIP_MultiScopes(t *testing.T) {
	...
	if err := s.Options(
		geoip.WithAlternativeHandler(scope.Store, 2, altHndlr), // for test case 2
		...
		geoip.WithErrorHandler(scope.Store, 2, func(err error) http.Handler {
			assert.Error(t, err, "%+v", err)
			return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
