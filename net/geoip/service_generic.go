...
func (s *Service) configByContext(ctx context.Context) (scpCfg ScopedConfig) {
	...
	if !scopeOK {
		scpCfg.lastErr = errors.NewNotFoundf("[geoip] configByContext: scope.FromContext not found")
		return
	}
	...
	if err := scpCfg.IsValid(); err != nil {
		...
		// made a mistake.
		scpCfg.lastErr = errors.Wrap(err, "[geoip] Service.configByContext.configFromScope") // rewrite error
	}
	return
}
...
func (s *Service) ConfigByScopedGetter(scpGet config.Scoped) ScopedConfig {
	...
	if s.optionFactory != nil {
		...
		if !ok { // unlikely to happen but you'll never know. how to test that?
			return newScopedConfigError(errors.NewFatalf("[geoip] Inflight.DoChan returned a closed/unreadable channel"))
		}
		if res.Err != nil {
			return newScopedConfigError(errors.Wrap(res.Err, "[geoip] Inflight.DoChan.Error"))
		}
		sCfg, ok := res.Val.(ScopedConfig)
		if !ok {
			sCfg = newScopedConfigError(errors.NewFatalf("[geoip] Inflight.DoChan res.Val cannot be type asserted to scopedConfig"))
		}
		return sCfg
	}
