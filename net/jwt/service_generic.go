...
func (s *Service) configByContext(ctx context.Context) (scpCfg ScopedConfig) {
	...
	if !scopeOK {
		scpCfg.lastErr = errors.NewNotFoundf("[jwt] configByContext: scope.FromContext not found")
		return
	}
	...
	if err := scpCfg.IsValid(); err != nil {
		...
		// made a mistake.
		scpCfg.lastErr = errors.Wrap(err, "[jwt] Service.configByContext.configFromScope") // rewrite error
	}
	return
}
