...
type (
	entityStoreMap map[uint64]*EavEntityStore
	once           struct {
		m    sync.Mutex
		done uint32
	}
...
func InitEntityStoreMap(dbrSess *dbr.Session) error {
	...
	if initMapDone.done == 0 {
		...
		s, err := GetTableStructure(TableEavEntityStore)
		if err != nil {
			return errgo.Mask(err)
		}
		var ess EavEntityStoreSlice
		_, err = dbrSess.
			Select(s.Columns...).
			From(s.Name).
			LoadStructs(&ess)
...
func (m entityStoreMap) Get(typeID, storeID int64) (*EavEntityStore, error) {
...
func (m entityStoreMap) Set(typeID, storeID int64, es *EavEntityStore) error {
	entityStoreMutex.Lock()
	defer entityStoreMutex.Unlock()
